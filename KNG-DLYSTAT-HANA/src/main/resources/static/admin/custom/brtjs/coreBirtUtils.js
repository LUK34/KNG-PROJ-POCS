...
export function dateValidator(sdate, edate, errorElement) 
{
    ...
    console.log("dateValidator-->Start Date-->"+startDate);
    console.log("dateValidator-->End Date-->"+endDate);
    console.log("dateValidator-->Current Date-->"+currentDate);
...
export function medicineDynaSingleFilter(officeId, medicineDropdownId) {
    document.getElementById(officeId).addEventListener('change', function() {
        var clinicId = this.value;
        fetch('./medicineList/' + clinicId)
            .then(response => response.json())
...
// ------------------------------- PHA TRANSACTION TYPE OPTIONS START ------------------------------- 
export function phaTransacTypeFilter(phaTransacTypedropdownId){
	const phaTransacTypeNames = ["Material Transfer","Purchase","Sale"];
    const phaTransacTypeDropdown = document.getElementById(phaTransacTypedropdownId);

	 phaTransacTypeNames.forEach((val, index) => {
        const option = new Option(val, index + 1); // The id starts from 1 for All
        phaTransacTypeDropdown.add(option);
    });

}

//	---- Material Transfer=1,Purchase=2,Sale=3  ----
// ------------------------------- PHA TRANSACTION TYPE  OPTIONS START ------------------------------- 



// --------------------------- 	UserType Options START ---------------------------------------------------
export function userTypeFilter(UserTypedropdownId){
	const UserTypeNames = ["All","Admin","Doctor","Nurse","Other","Radiographer Technologist","Radiographer Technician"];
    const UserTypeDropdown = document.getElementById(UserTypedropdownId);

	 UserTypeNames.forEach((val, index) => {
        const option = new Option(val, index + 1); // The id starts from 1 for All
        UserTypeDropdown.add(option);
    });

}
// ----- All=1, Admin=2, Doctor=3, Nurse=4, Other=5, Radiographer Technologist=6, Radiographer Technician=7

// --------------------------- UserType Options END ---------------------------------------------------


// ------------------------------- KNG/MOH START ---------------------------------------------------
export function kngMohFilter(kngMohdropdownId){
	const kngMohNames = ["All","KNG","MOH"];
    const kngMohDropdown = document.getElementById(kngMohdropdownId);

	 kngMohNames.forEach((val, index) => {
        const option = new Option(val, index + 1); // The id starts from 1 for All
        kngMohDropdown.add(option);
    });
...
// ------------------------------- Status START ---------------------------------------------------
export function statusFilter(statusdropdownId){
	const statusNames = ["All","Active","Inactive"];
    const statusDropdown = document.getElementById(statusdropdownId);

	 statusNames.forEach((val, index) => {
        const option = new Option(val, index + 1); // The id starts from 1 for All
        statusDropdown.add(option);
    });

}
// ----- All=1, Active=2, Inactive=3
// ------------------------------- Status END ---------------------------------------------------
